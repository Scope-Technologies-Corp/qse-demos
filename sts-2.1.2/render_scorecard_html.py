...
Render scorecard.json into a professional PDF-style HTML report.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render investor-style HTML scorecard")
...
<head>
  <meta charset="utf-8" />
  <title>NIST STS Entropy Scorecard</title>
  <style>
    body {{
  ...
  <div class="page">
    ...
    <div class="grid">
      <div class="card">
        ...
        <div class="muted">Passed Tests: {sc["qse_summary"]["passed_tests"]}/{sc["qse_summary"]["total_tests"]}</div>
      </div>

      <div class="card">
        <h3>System Overall</h3>
        <p class="value">{badge("PASS", "pass") if sys_pass else badge("FAIL", "fail")}</p>
        <div class="muted">Passed Tests: {sc["system_summary"]["passed_tests"]}/{sc["system_summary"]["total_tests"]}</div>
      </div>
    ...
    <div class="section">
      ...
      <table>
        <tr>
          ...
          <th>Weakest Test</th>
          <th>Pass Rate</th>
          <th>Uniformity p-value</th>
          <th>Threshold Margin</th>
        </tr>
        <tr>
          <td><b>QSE</b></td>
          <td>{sc["qse_weakest_test"]["test_name"]}</td>
          <td>{sc["qse_weakest_test"]["passed"]}/{sc["qse_weakest_test"]["total"]} ({fmt_pct(sc["qse_weakest_test"]["pass_rate"])})</td>
          <td>{sc["qse_weakest_test"]["p_value_uniformity"]}</td>
          <td>{sc["qse_weakest_test"]["passed"] - sc["qse_weakest_test"]["min_required"]}</td>
        </tr>
        <tr>
          <td><b>System</b></td>
          <td>{sc["system_weakest_test"]["test_name"]}</td>
          <td>{sc["system_weakest_test"]["passed"]}/{sc["system_weakest_test"]["total"]} ({fmt_pct(sc["system_weakest_test"]["pass_rate"])})</td>
          <td>{sc["system_weakest_test"]["p_value_uniformity"]}</td>
          <td>{sc["system_weakest_test"]["passed"] - sc["system_weakest_test"]["min_required"]}</td>
        </tr>
      </table>
      <div class="small" style="margin-top:8px;">
        Note: STS evaluates statistical randomness. It does not alone certify cryptographic strength or “quantum resilience.”
      </div>
    </div>
...
"""
    ...
    for row in sc["comparisons"]:
        ...
        <tr>
          <td>{row["test_name"]}</td>
          <td>{row["qse"]["passed"]}/{row["qse"]["total"]} ({fmt_pct(row["qse"]["pass_rate"])})</td>
          <td>{row["qse"]["p_value_uniformity"]}</td>
          <td>{row["system"]["passed"]}/{row["system"]["total"]} ({fmt_pct(row["system"]["pass_rate"])})</td>
          <td>{row["system"]["p_value_uniformity"]}</td>
...
"""
    ...
    <div class="section">
      ...
      <div class="muted">
        ...
        • Track stability: count how often any test hits minimum thresholds across runs.<br/>
        • Archive all STS reports and parameters for auditability.
      </div>
    </div>
